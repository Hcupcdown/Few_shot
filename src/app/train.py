...
class Trainer():
    ...
    def train(self):
        ...
        for epoch in range(self.args.epoch):
            ...
            self.model.train()
            train_losses = self.train_epoch(self.train_loader, epoch=epoch)
            ...
            print("epoch: {:03d} | trn loss: {:.8f} | trn snr: {:.8f}"
                  .format(epoch, train_loss, train_losses["snr"]))

            if not self.args.few_shot:
                continue
            self.model.eval()
            with torch.no_grad():
                losses =self.test_epoch(self.val_loader,
                                        epoch=epoch)
            ...
            print("epoch: {:03d} | val loss: {:.8f} | snr: {:.8f}"
                   .format(epoch, losses["loss"], losses["snr"]))
